...
export default defineNuxtPlugin(async (nuxtApp) => {
    ...
    async function uploadImage(file: Blob | Uint8Array | ArrayBuffer, fileName: any, folder='images') {
  ...
  async function getImagesFromDirectory(directory: string | undefined) {
    ...
    async function getCollectionData(collectionName: string, order:string) {
        ...
        if(order.length > 0) {
           querySnapshot = await getDocs(query(collection(firestore, collectionName), orderBy(order)));
        }else {
           querySnapshot = await getDocs(query(collection(firestore, collectionName)));
        }
